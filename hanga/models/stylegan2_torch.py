...
from hanga.models.ops import FusedLeakyReLU, fused_leaky_relu, upfirdn2d
...
class PixelNorm(nn.Module):
    ...
    def forward(self, inputs):
        return inputs * torch.rsqrt(torch.mean(inputs**2, dim=1, keepdim=True) + 1e-8)
...
class Generator(nn.Module):
    def __init__(
        ...
        if checkpoint is not None:
            self.load_state_dict(torch.load(checkpoint)["g_ema"])
        ...
        if size != output_size or base_res_factor != 1:
            for layer_idx in range(self.num_layers):
                ...
                setattr(self.noises, f"noise_{layer_idx}", torch.randn(*shape))
    ...
    def forward(
        ...
        if map_latents:
            latent = torch.cat([self.style(s[None, None, :]) for s in styles], axis=0)
        ...
        if isinstance(truncation, float):
            truncation = torch.cuda.FloatTensor([truncation])
